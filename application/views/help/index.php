<br />
<div class="hero-unit">
   <h1>Help</h1>
   <p>Brovine is designed for the Cal Poly Animal Science department, and it allows
   them to catalog and analyze genetic information.</p>
   <p><a class="btn btn-default" href="/help/ProjectGoals">Learn More</a></p>
</div>

<div class="row-fluid">
   <ul class="thumbnails">
      <li class="span6">
         <h2><a href="/help/UsingBrovine">Using Brovine</a></h2>

         <h3>View Descriptions</h3>
         <ul>
            <li><a href="/help/UsingBrovine#experiment_hierarchy">
               Experiment Hierarchy</a></li>
            <li><a href="/help/UsingBrovine#tf_search">
               Transcription Factor Search</a></li>
            <li><a href="/help/UsingBrovine#tf_subtract">
               Transcription Factor Subtract</a></li>
            <li><a href="/help/UsingBrovine#gene_summary">
               Gene Summary</a></li>
            <li><a href="/help/UsingBrovine#tf_summary">
               Transcription Factor Summary</a></li>
            <li><a href="/help/UsingBrovine#gene_search">
               Gene Search</a></li>
            <li><a href="/help/UsingBrovine#tf_popularity">
               Transcription Factor Popularity</a></li>
            <li><a href="/help/UsingBrovine#freq_transcription_factors">
               Frequent Transcription Factors</a></li>
         </ul>

         <h3><a href="/help/UsingBrovine">Account Management</a></h3>
         <ul>
            <li>Privilege Types</li>
            <li>Adding/Removing User Accounts</li>
            <li>Changing your Password</li>
            <li>Changing your Display Name</li>
         </ul>
      </li>

      <li class="span6">
         <h2>For Brovine Developers</h2>

         <h3>Brovine</h3>
         <ul>
            <li><a href="/help/Technical#overview">Techincal Overview</a></li>
            <li><a href="/help/Technical#frontend">Front-end Description</li>
            <li><a href="/help/Technical#backend">Back-end Description</li>
            <li><a href="/help/Technical#config">Server Configuration</li>
            <li><a href="/help/SQLSchema">SQL Schema Description</li>
            <li><a href="http://github.com/tcirwin/teambrovine">The GitHub repository</a>
         where Brovine is stored.</a></li>
         </ul>

         <h3><a href="/help/DevSetup">Setting Up a Development Box</a></h3>

         <h3>Frequent Itemset Generator</h3>
         <ul>
            <li><a href="/help/FreqItemsetGen#overview">Technical Overview</a></li>
            <li><a href="http://github.com/tcirwin/freq-itemset-gen">The GitHub repository</a>
         where the Frequent Itemset Generator used by Brovine is stored.</a></li>
         </ul>

         <h3><a href="/help/Glossary">Brovine Glossary</a></h3>
      </li>
   </ul>
</div>
